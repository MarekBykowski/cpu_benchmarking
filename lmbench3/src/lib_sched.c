...
sched_pin(int cpu)
{
	...
	/* Linux interface */
	static cpu_set_t* mask = NULL;
	static cpu_set_t* cpumask = NULL;
	static int sz = 0;
	static int ncpus = 0;
	int i;
	int j;
	...
	if (cpumask == NULL) {
		...
		mask = (cpu_set_t*)malloc(sz * sizeof(unsigned long));
		cpumask = (cpu_set_t*)malloc(sz * sizeof(unsigned long));
		...
		for (i = 0; i < sz * 8 * sizeof(unsigned long); ++i) {
			...
			if (cpumask->__bits[word] & (1 << bit)) ncpus++;
		}
	}
	cpu %= ncpus;
	...
	for (i = 0, j = 0; i < sz * 8 * sizeof(unsigned long); ++i) {
		int	word = i / (8 * sizeof(unsigned long));
		int	bit = i % (8 * sizeof(unsigned long));
		if (cpumask->__bits[word] & (1 << bit)) {
			if (j >= cpu) {
				mask->__bits[word] |= (1 << bit);
				break;
			}
			j++;
		}
