...
init_loop(iter_t iterations, void *cookie)
{
	...
	printf("mb: buff allocated %p - %p, size %lx \n",
		(void*)state->buf, (void*) ((char*) state->buf + state->nbytes + 512),
	...
	memset((void*)state->buf, 0xab, state->nbytes);
...
{	
	state_t *state = (state_t *) cookie;
	register volatile unsigned *lastone asm("x18");
	lastone = state->lastone;
	register int sum = 0;

	while (iterations-- > 0) {
	    register volatile unsigned *p asm("x19");
	    p = state->buf;
	    while (p <= lastone) {
#if 0
		sum += 
