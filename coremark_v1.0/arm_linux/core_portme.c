...
static int key_id=0;
ee_u8 core_start_parallel(core_results *res) {
	key_t key=74321+key_id;
	key_id++;
	res->port.pid=fork();
	res->port.shmid=shmget(key, 10, IPC_CREAT | 0666);
	if (res->port.shmid<0) {
		ee_printf("ERROR in shmget!\n");
	}
	if (res->port.pid==0) { /*child*/
		unsigned cpu;
		iterate(res);
#if MANAGE_AFFINITY
		for (int i=4; i<=8; i++)
		...
		if (sched_setaffinity(getpid(), sizeof(cpuset), &cpuset) == -1)
			ee_printf("ERROR in sched_setaffinity!\n");
#endif
		if ((res->cpu = (unsigned) sched_getcpu()) == -1)
			ee_printf("ERROR in sched_getcpu!\n");
		...
		if (res->port.shm == (char *) -1) {
			...
			memcpy(res->port.shm,&(res->crc),10);
			shmdt(res->port.shm);
		}
		exit(0);
	}
...
ee_u8 core_stop_parallel(core_results *res) {
	...
	memcpy(&(res->crc),res->port.shm,10);
	shmdt(res->port.shm);
	return 1;
}
#elif USE_SOCKET
